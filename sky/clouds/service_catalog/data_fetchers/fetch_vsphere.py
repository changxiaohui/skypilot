...
PRESET_GPU_LIST = [
    {
        ...
        'MemoryMB': 1024 * 16,
        'vCPUs': 8,
        'fullNames': ['Tesla V100', 'Tesla V100-PCIE-16GB',
                      'Tesla V100-PCIE-32GB', 'Tesla V100-SXM2-16GB']
    },
    {
        'Model': 'P100',
        'Type': 'GPU',
        ...
        # Estimated
        'vCPUs': 8,
        'fullNames': ['Tesla P100-PCIE-16GB', 'Tesla P100-PCIE-12GB',
                      'Tesla P100-SXM2-16GB', 'Tesla P100-SXM2-12GB']
    },
    {
        'Model': 'T4',
        'Type': 'GPU',
    ...
    },
    {
        ...
        # Estimated
        'vCPUs': 4,
        'fullNames': ['GeForce RTX 2080 Ti', 'GeForce RTX 2080 SUPER',
                      'GeForce RTX 2080']
    },
    {
        'Model': 'A5000',
    ...
    try:
        with open(csv_saving_path, 'a', encoding='utf-8') as f:
            for host in hosts_for_csv:
                f.write(
                    f'{host.get("HostName")},{host.get("MoRefID")},'
...
def summarize_by_cluster_and_gpu(
    ...
    try:
        ...
        for host in hosts_to_summarize:
            ...
            # the cluster
            if cluster_name not in weakest_cpu_host_of_each_cluster:
                weakest_cpu_host_of_each_cluster[cluster_name] = host.get(
                    'TotalCPUcores', 0)
            else:
                if host.get('TotalCPUcores') and host.get('TotalCPUcores') < \
                    ...
                    weakest_cpu_host_of_each_cluster[cluster_name] = host.get(
                        'TotalCPUcores', 0)
        return {'gpu_cluster_dict': gpu_cluster_dict,
                'weakest_cpu_host_of_each_cluster':
                    weakest_cpu_host_of_each_cluster}
    except Exception as e:
                ...
                for cluster_name in gpu_clusters_dict:
                    ...
                    if gpus_dict:
                        for gpu_name in gpus_dict:
                            ...
                            f.write(
                                f'gpu.{gpu_short_name},{gpu_short_name},'
                                f'{gpu_info["AcceleratorCount"]},'
                                f'{gpu_info["vCPUs"]},'
                                f'{round(gpu_info["MemoryMB"] / 1024, 2)},'
                                f'\"{gpu_info["GpuInfo"]}\",0.0,0.0,'
                                f'{vcenter_name},{cluster_name}\n')
            cpu_host_dict = summary_result.get(
                'weakest_cpu_host_of_each_cluster')
            if cpu_host_dict:
                for cluster_name in cpu_host_dict:
                    ...
                    for instance_type in CPU_INSTANCE_TYPES:
                        if instance_type.get(
                                'vCPUs') <= minimal_cpu_cores:  # pylint: disable=invalid-name
                            vCPUs = instance_type.get(  # pylint: disable=invalid-name
                                'vCPUs')
                            memoryMB = instance_type.get( # pylint: disable=invalid-name
                                'MemoryMB', 0)
                            # Make sure that memoryMB is a number before
                            # proceeding
                            if isinstance(memoryMB, (int, float)):
                                memory_in_gb = round(memoryMB / 1024, 2)
                            else:
                                raise TypeError(
                                    f'The value for \'MemoryMB\' is not a '
                                    f'number: {memoryMB}')
                            f.write(
                                f'{instance_type.get("InstanceType")},,,'
...
def initialize_images_csv(csv_saving_path: str, vc_object,
    ...
    vc_tag_obj = Tag(vc_stub_config)
    vc_tag_association_obj = TagAssociation(vc_stub_config)
        ...
        for lib_id in content_library_ids:
            ...
            for item_id in library_item_ids:
                ...
                if item.type == 'vm-template':
                    ...
                    continue
                dynamic_id = DynamicID(type='com.vmware.content.library.Item',
                                       id=item.id)
                ...
                for tag_id in associated_tags:
                    ...
                    if 'SKYPILOT-CPU' in tag_name.upper():
                        if not cpu_image_added:
                            f.write(
                                f'{item.id},{vcenter_name},{item_cpu},'
                                f'{item_memory},,,\'[]\'\n')
                ...
                if len(gpu_tags) > 0:
                    gpu_tags = str(gpu_tags).replace("\'", "\"")
                    f.write(
                        f'{item.id},{vcenter_name},{item_cpu},{item_memory}'
                        f',,,\'{gpu_tags}\'\n')
...
def initialize_instance_image_mapping_csv(vms_csv_path: str,
    ...
    # Merging on vCenter
    merged_df = pd.merge(vms_df, images_df, left_on='Region',
                         right_on='vCenter')
    results = []
    ...
    for _, row in merged_df.iterrows():
        if pd.isna(row['AcceleratorName']) and row['GpuTags'] == []:
            ...
            if not pd.isna(row['AcceleratorName']):
                ...
                vm_gpu_manufacturer_fullname = \
                    json.loads(row['GpuInfo'].replace('\'', '\"'))['Gpus'][0][
                        'Manufacturer'].lower()
                ...
                for image_gpu_tag in row['GpuTags']:
                    ...
                    if image_gpu_tag_lower in vm_gpu_fullname:
                        results.append([row['InstanceType'], row['ImageID'],
                                        row['vCenter']])
    ...
    result_df.to_csv(csv_saving_path, mode='a', header=False, index=False,
                     encoding='utf-8')
