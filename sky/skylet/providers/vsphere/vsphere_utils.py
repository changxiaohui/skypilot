...
from pyVmomi import vim, pbm, VmomiSupport  # type: ignore
...
from sky.clouds.service_catalog.data_fetchers.fetch_vsphere import \
    initialize_accelerators_csv, initialize_hosts_csv, \
    initialize_images_csv, initialize_instance_image_mapping_csv, \
    initialize_vms_csv
from sky.skylet.providers.vsphere.common.id_generator import \
    generate_random_uuid
from sky.skylet.providers.vsphere.common.service_manager_factory import \
    ServiceManagerFactory
from sky.skylet.providers.vsphere.common import vim_utils
from sky.skylet.providers.vsphere.common.cls_api_client import ClsApiClient
from sky.skylet.providers.vsphere.common.cls_api_helper import ClsApiHelper
from sky.skylet.providers.vsphere.common.vim_utils import poweron_vm, \
    wait_for_tasks, \
    wait_internal_ip_ready, create_spec_with_script
...
class VsphereClient:
    def __init__(self, server: str, username: str, password: str,
        ...
        self.Tag = None
        self.TagAssociation = None

    def connect(self):
        if not self.servicemanager:
            self.servicemanager = (
                ServiceManagerFactory.get_service_manager(self.__server,
                                                          self.__username,
                                                          self.__password,
                                                          self.
                                                          skip_verification))
        ...
        if service_manager:
            if not instance_uuids:
                ...
                for inst_id in instance_uuids:
                    instance_obj = (service_manager.content.searchIndex.
                                    FindByUuid(None, inst_id, True, True))
                    if instance_obj:
            ...
            for inst in instances:
                ...
                if inst.name.startswith(VM_PREFIX):
                    ...
                    cust_attributes = [(f.name, v.value) for f
                                       in inst.availableField if f.name in
                                       filter_keys
                                       for v in inst.customValue if
                                       f.key == v.key]
                    for filter in filters:
                        if ((filter['Key'], filter['Value']) not in
                                cust_attributes):
                            break
                    else:
    ...
    def create_instances(self, cluster: str, host_mobid: str,
                         lib_item_id: str, spec: vim.vm.ConfigSpec,
        ...
        if service_manager:
            ...
            cluster_obj = vim_utils.get_objs_by_names(service_manager.content, [
            ...
            if profile_id is None:
                logger.error(
                    "The VM storage policy 'skypilot_policy' is not available.")
                raise exceptions.ResourcesUnavailableError(
                    "The VM storage policy 'skypilot_policy' is not available.")
            # Deploy the ovf template
            vm_obj = self.helper.deploy_ovf_template(vm_name=self.vm_name,
                                                     lib_item_id=lib_item_id,
                                                     ovf_summary=ovf_summary,
                                                     deployment_target=deployment_target,
                                                     storage_profile_id=profile_id)

            customization_spec = create_spec_with_script(vm_obj,
                                                         customization_spec_str)
            check_customization_spec = vm_obj.CheckCustomizationSpec(
                spec=customization_spec)
            if check_customization_spec:
                logger.error(
                    f'Check customization spec failed for VM {vm_obj.name}: {check_customization_spec}')
                raise exceptions.ResourcesUnavailableError(
                    f'Check customization spec failed for VM {vm_obj.name}: {check_customization_spec}')
            wait_for_tasks(self.client.service_manager.content,
        ...
        if service_manager:
            ...
            for tag in tags:
                if tag['Key'] not in fields:
                    new_field = cfm.AddFieldDefinition(tag['Key'],
                                                       vim.VirtualMachine)
                    cfm.SetField(vm, new_field.key, tag['Value'])
                    continue
                exi_key = [fld.key for fld in vm.availableField if
                           fld.name == tag['Key']][0]
...
def initialize_vsphere_data():
    ...
    for vcenter in get_vsphere_credentials():
        ...
        vc_object = VsphereClient(vcenter['name'],
                                  vcenter['username'],
                                  vcenter['password'],
                                  vcenter['clusters'],
