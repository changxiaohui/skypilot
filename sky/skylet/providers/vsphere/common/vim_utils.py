...
from sky.clouds.service_catalog.data_fetchers.fetch_vsphere import get_accelerators_from_csv
...
def get_objs_by_names(content: vim.ManagedEntity,
                      vimtype: type,
                      names: List[str]) -> Dict[str, vim.ManagedEntity]:
    """
...
def get_hosts_by_cluster_names(content, vcenter_name, cluster_name_dicts=None):
    ...
    if not cluster_name_dicts:
        ...
        cluster_name_dicts = [{"name": cluster.name} for
                              cluster in cluster_view.view]
    ...
    cluster_names = [cluster_dict["name"] for cluster_dict in
                     cluster_name_dicts]

    # Use get_objs to retrieve all cluster objects at once
    cluster_obj_dict = get_objs_by_names(
        content, [vim.ClusterComputeResource], cluster_names)
    ...
    for cluster_name_dict in cluster_name_dicts:
        ...
        if cluster_obj:
            ...
            hosts_info = list_hosts_with_devices_info(
                cluster_obj.host, vcenter_name, datacenter_name,
...
def list_hosts_with_devices_info(
        hosts: List[vim.HostSystem],
        vcenter_name, datacenter_name, cluster_name, gpu_list):
    def decimal_to_four_char_hex(decimal):
        return f"{str(hex(decimal)).replace('0x', '').zfill(4)}"
    ...
    for host in hosts:
        ...
        available_memory_mb = round(memory_info_mb -
                                    quick_stats.overallMemoryUsage, 2)
        ...
        for device, passthru_info in zip(pci_devices, pci_passthru_devices):
            if (decimal_to_four_char_hex(device.classId)[
               :2] in DISPLAY_CONTROLLER_CLASS_ID_PREFIXES and
                ...
                for preset_accelerator in gpu_list:
                    if (preset_accelerator["Model"].lower() in
                            device.deviceName.lower()):
                        memory_info = {"SizeInMiB":
                                           preset_accelerator["MemoryMB"]}
                        break
                device_info = {
                    "ID": device.id,
                ...
                # GpuInfo is for vms.csv, which only needs one entry for each host
                # Always put the GPU info into the vms.csv (
                # there are some inconsistent
                # So we should check the host's accelerators filed for the GPU status
...
def wait_for_tasks(content: vim.ServiceInstanceContent, tasks: List[vim.Task]):
    """
...
def create_spec_with_script(
        vm: vim.VirtualMachine,
        script_string: str) -> vim.vm.customization.Specification:
    hostname = vim.vm.customization.FixedName()
    hostname.name = vm.name
    identity = vim.vm.customization.LinuxPrep(hostName=hostname,
                                              domain="VMWare.com",
                                              scriptText=script_string)
    global_ip_settings = vim.vm.customization.GlobalIPSettings()
    ip = vim.vm.customization.DhcpIpGenerator()
    ip_settings = vim.vm.customization.IPSettings(ip=ip)
    nic_setting_maps = []
    for device in vm.config.hardware.device:
        if isinstance(device, vim.vm.device.VirtualEthernetCard):
            nic_setting_map = vim.vm.customization.AdapterMapping(
                adapter=ip_settings)
...
def get_vm_uuid_from_vm():
    ...
    if matches:
        ...
        if len(serial_number) != 32:
            logger.error("Invalid serial number: ", serial_number)
