...
from sky import status_lib, exceptions, sky_logging
from sky.skylet.providers.vsphere import vsphere_utils
from sky.skylet.providers.vsphere.common.metadata_utils import Metadata
from sky.skylet.providers.vsphere.common.vim_utils import poweroff_vm, \
    wait_internal_ip_ready, wait_for_tasks
from sky.skylet.providers.vsphere.common.custom_script import CUSTOMIZED_SCRIPT
from sky.skylet.providers.vsphere.vsphere_utils import VsphereClient
...
def run_instances(region: str, cluster_name: str,
    ...
    if to_start_num > 0:
        ...
        vsphere_cluster_name = _choose_vsphere_cluster_name(config, region,
                                                            vc_object)
    ...
    return common.ProvisionRecord(provider_name='vsphere',
                                  region=region,
...
def _create_instances(cluster_name: str, config: common.ProvisionConfig,
                      region: str, vc_object: VsphereClient,
                      vsphere_cluster_name: str):
    """Create vsphere VM instances"""
    lib_item_id = None
    ...
    if not gpu_instance:
        ...
        if len(image_instance_mapping_df) == 0:
            raise Exception(
                ...
                SKYPILOT-NVIDIA.""")
        image_instance_mapping_item = image_instance_mapping_df.iloc[
            0].to_dict()
    ...
    assert lib_item_id is not None, (f'Failed to get the lib item id for '
    ...
    hosts_df = hosts_df[
        (hosts_df['AvailableCPUs'] / hosts_df['cpuMhz']) >= cpus_needed]
    hosts_df = hosts_df[hosts_df['AvailableMemory(MB)'] >= memory_needed]
    assert len(
        hosts_df) > 0, (f'There is no host available to create the instance '
            ...
            if gpus != '[]':
                ...
                for gpu in gpus:
                    if gpu.get('Status') == 'Available':
                        if vms_item['AcceleratorName'].lower() in gpu.get(
                                'DeviceName').lower():
    ...
    if gpu_instance and gpu:
        ...
        pci_device_spec = vim.vm.device.VirtualDeviceSpec()
        pci_device_spec.operation = (vim.vm.device.VirtualDeviceSpec.
                                     Operation.add)

        backing = vim.vm.device.VirtualPCIPassthrough.DynamicBackingInfo()
        allowed_device = vim.vm.device.VirtualPCIPassthrough.AllowedDevice()
    ...
    created_instance_uuid = vc_object.create_instances(
        cluster=vsphere_cluster_name, host_mobid=host_mobid,
        lib_item_id=lib_item_id, spec=spec,
        customization_spec_str=custom_script)
    if created_instance_uuid is None:
        logger.error(
            f'Failed to create the instance on host {host_mobid} in '
...
def _choose_vsphere_cluster_name(config: common.ProvisionConfig, region: str,
    ...
    if len(vc_object.clusters) > 0:
        for optimized_cluster_name in vsphere_cluster_name_str.split(','):
            if optimized_cluster_name in [item['name'] for item in
                                          vc_object.clusters]:
...
def _get_vc_object(region):
    ...
    vc_object = vsphere_utils.VsphereClient(vcenter['name'],
                                            vcenter['username'],
                                            vcenter['password'],
                                            vcenter['clusters'],
                                            vcenter[skip_key])
...
def _get_filtered_instance(vc_object, cluster_name_on_cloud: str,
...
def stop_instances(
    ...
    vc_object = _get_vc_object(region)
    vc_object.connect()
    instances = _get_filtered_instance(vc_object,
...
def terminate_instances(
    ...
    if not instances:
        return
    vm_service = VM(vc_object.servicemanager.stub_config)
    for inst in instances:
        if inst.runtime.powerState == 'poweredOn':
            poweroff_vm(vc_object.servicemanager.content, inst)
...
def _get_head_instance_id(instances):
    ...
    for inst in instances:
        cust_attributes = [(f.name, v.value) for f in inst.availableField if
                           f.name == head_node_filter['Key']
                           for v in inst.customValue if f.key == v.key]
        if (
                head_node_filter['Key'],
                head_node_filter['Value']) in cust_attributes:
